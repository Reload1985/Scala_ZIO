...
// STM = "atomic effect"
  ...
  * */
  ...
  // withPermit
  val semWithPermit: UIO[Int] = tSemaphoreEffect.commit.flatMap {
    sem => sem.withPermit {
      ZIO.succeed(42)
    }
  }

  // TReentrantLock - can acquire the lock multiple times without deadlock
  ...
  def demoReadersWriters(): UIO[Unit] = {
    ...
    for {
      ...
      readerFib <- ZIO.collectAllParDiscard((1 to 10).map(read(_, lock))).fork
