...
object Refs extends ZIOAppDefault {
  ...
  // obtain a value
  val mol = atomicMOL.flatMap{
    ref => ref.get // returns a UIO[Int], thread-safe getter
  }

  // changing
  val setMol = atomicMOL.flatMap{
    ref => ref.set(100) // UIO[Unit], thread-safe setter
  }
  ...
  val updateMol = atomicMOL.flatMap{
    ref => ref.update(_ * 100)
  }
  ...
  // modify
  val modifieMol: UIO[String] = atomicMOL.flatMap{ ref =>
  ...
  def demoConcurrentWorkImpure(): UIO[Unit] = {
    ...
    def task(workload: String): UIO[Unit] = {
      ...
      for {
        ...
        _ <- ZIO.succeed(count += wordCount) // updating variable
      } yield()
    }
  ...
  * */
  ...
  // update function may be run more then once
  def demoMultiUpdates: UIO[Unit] = {
