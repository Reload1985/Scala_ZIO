...
object Promises extends ZIOAppDefault {
  ...
  val writer = aPromise.flatMap{
    promise => promise.succeed(42)
  }
  ...
  * */
  ...
  def downloadFileWithRef(): Task[Unit] = {
    def downloadFile(contentRef: Ref[String]): Task[Unit] =
      ZIO.collectAllDiscard(
        fileParts.map {
          part => ZIO.succeed(s"got '$part'").debugThread *> ZIO.sleep(1.second) *> contentRef.update(_ + part)
        }
      )
  ...
  def downloadFileWithPromise(): Task[Unit] = {
  ...
  *   - one waits for the counter to become 10, after wich it will "ring a bell"
  *
  * 2. Write a "race pair"
  *   - use a Promise which can hold an Either[exit for A, exit for B]
  *   - start a fiber for each ZIO
  *   - on completition (with any status), each ZIO needs to complete that Promise
  *   hint => use finalizer
  ...
  *    */
  ...
  val demoRacePair = {
    ...
    val ziob = ZIO.sleep(2.seconds).as(2).onInterrupt(ZIO.succeed("second interrupted").debugThread)
